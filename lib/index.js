 ...
 */
const getJwtToken = () => {
  if (isEmpty(jwtToken) && common.isBrowser) {
    jwtToken = sessionStorage.getItem('token'); // eslint-disable-line
  }

  if (isEmpty(jwtToken) && common.isNode) {
    jwtToken = env.getString('EWEA_API_TOKEN');
  }
...
const getAuthenticatedParty = () => {
  if (isEmpty(party) && common.isBrowser) {
...
const prepareParams = (params) => {
  ...
  const filters = common.mergeObjects(options.filter);
  options.filter = mapValues(filters, (val) => {
    ...
    // clear empty object
    if (isPlainObject(val) && isEmpty(common.mergeObjects(val))) {
      return undefined;
    }
    // array
    if (isArray(val)) {
  ...
  const cleanParams = common.mergeObjects(options);
  return cleanParams;
};
    ...
    [methodName]: (payload) => {
      ...
      const data =
        payload instanceof FormData
          ...
          : common.mergeObjects(payload, defaults);
      // derive endpoint
      let endpoint = `/${toLower(plural)}/${common.idOf(data)}`;
      if (!isEmpty(bucket)) {
        endpoint = `/${toLower(plural)}/${bucket}/${common.idOf(data)}`;
      }
      // issue http request
